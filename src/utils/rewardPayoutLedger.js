...
const CURRENT_QUIZ_ADDRESS = normalizeAddress(quiz_address);

function isCurrentQuizEntry(entry = {}) {
    const sourceAddress = normalizeAddress(entry?.sourceAddress || quiz_address);
    return !sourceAddress || sourceAddress === CURRENT_QUIZ_ADDRESS;
}
...
function readRewardPayoutEntries() {
    try {
        ...
        return localEntries.filter(isCurrentQuizEntry);
    } catch (error) {
...
function writeRewardPayoutEntries(entries = []) {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(normalizeEntries(entries).filter(isCurrentQuizEntry)));
}
